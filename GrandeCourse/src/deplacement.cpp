...
Deplacement::Deplacement(int speed)
{
	m_speed = speed;
}


void Deplacement::vAvancer()
{
	//LCD_Printf("%i \n" , m_speed);
	//ucVitesse += iPID();
	//_iVitesse = ucVitesse;
	MOTOR_SetSpeed(MOTOR_LEFT,m_speed);
	MOTOR_SetSpeed(MOTOR_RIGHT,VITD);
	//float iNbEncodeur = dist/dCm_Coche;
	//while(compteurD <= iNbEncodeur && on == true)
	//{
		m_speed += iPID();
...
void Deplacement::vTourner(int iAngle, int iMoteur_STOP, int iMoteur_RUN, int iEncodeur_RUN)
{
	...
	if (iNbCochesTotal < 0) // Si l'angle est n�gatif (Pour reculer)
		iNbCochesTotal *= -1;
